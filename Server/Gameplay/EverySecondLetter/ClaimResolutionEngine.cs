...
public sealed class ClaimResolutionEngine
{
  ...
  public ClaimResolutionEngine(EverySecondLetterGameDefinition definition)
  {
      ...
      bool isValidWord)
  {
    ...
    var baseScore = _definition.GetBaseScore(contributionCount);
  ...
  {
    if (!disputed)
      return new List<PlayerPoints> { new(claimerId, _definition.GetAcceptedScore(baseScore)) };

    if (isValidWord)
      return new List<PlayerPoints> { new(claimerId, _definition.GetValidDisputedScore(baseScore)) };

    return new List<PlayerPoints> { new(responderId, _definition.GetInvalidDisputedScore(baseScore)) };
  }
